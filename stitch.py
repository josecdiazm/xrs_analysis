...
def stitching(datas, ais, masks, geometry='Reflection', interp_factor=2,
    ...
    :param ais: List of FiberIntegrator objects (promoted from AzimuthalIntegrator)
    ...
    if _cached_qranges is None:
        ...
        for i, (ai, mask) in enumerate(zip(ais, masks)):
            ...
            qp_start = np.argmin(abs(qp_remesh - np.nanmin(q_p_ini[:, i])))
            qp_stop  = np.argmin(abs(qp_remesh - np.nanmax(q_p_ini[:, i])))
        ...
        if i == 0:
            ...
            if flag_scale:
                if ai.detector.aliases[0] != 'Pilatus 900kw (Vertical)':
                    sca2[:, qp_start: qp_start + np.shape(qimage)[1]] += (
                        qimage >= threshold).astype(int) * scale
                    scales.append(scale)
                else:
                    if i % 3 == 0:
                        sca3 = np.zeros(np.shape(img_te))
